...
impl HttpsProvider {
  ...
  pub fn create_session(&mut self, frontend_sock: TcpStream, token: ListenToken, poll: &mut Poll, session_token: Token, timeout: Timeout) -> Result<(Rc<RefCell<dyn ProxySessionCast>>,bool), AcceptError> {
    match self {
